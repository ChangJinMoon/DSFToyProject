...
import { deleteProject, sprintinit } from "../../redux/_actions/user_actions";
...
function PersonalProject() {
  ...
  useEffect(() => {
    ...
    const sprint = async () => {
      dispatch(sprintinit(userId, projectId)).then((res) => {
        console.log(res);
    ...
    <div>
      {array ? (
        <>
          ...
          <Link to={`/EditPage/${array.projectId}`}>
            <button>수정</button>
          </Link>
          <button
            onClick={() => {
          ...
          <Link to={`/SprintPage/${array.projectId}`}>
